...
def convert_to_llama_format(sample):
    # chosen_sentence += end_of_conversation_token / We will add eos token later "</s>"
    sample_parts = re.findall(r'(Human:.*?)(?=Human:|Assistant:|$)|(Assistant:.*?)(?=Human:|Assistant:|$)', sample, re.DOTALL)
    formatted_sample = "<s>[INST] <<SYS>>\nYou are a helpful assistant. You will be given a user's question, and you need to answer it.\n<</SYS>>\n\n"
  
    human_count = 0
    for human, assistant in sample_parts:
        if human:
            # first sentence
            if human_count == 0:
                formatted_sample += human.replace("Human:", "").strip() + " [/INST] "
                human_count += 1
            # later ones
            else:
                formatted_sample += "</s><s>[INST] " + human.replace("Human:", "").strip() + " [/INST] "
                human_count += 1
        if assistant:
            formatted_sample += assistant.replace("Assistant:", "").strip()
    return formatted_sample

def save_subset_to_json(subset, file_path):
    if not isinstance(subset, Subset):
        raise ValueError("The provided dataset is not a Subset object.")

    data = [subset.dataset[idx] for idx in subset.indices]
    os.makedirs(os.path.dirname(file_path), exist_ok=True)
    with open(file_path, 'w') as f:
        for idx in subset.indices:
            sample = json.dumps(subset.dataset[idx])
            f.write(sample + '\n')
...
class DahoasFullhhrlhfDataset(PromptRawDataset):
    ...
    def get_prompt_and_chosen(self, sample):
        #return sample['prompt'] + sample['chosen']
        return convert_to_llama_format(sample['prompt'] + sample['chosen'])
...
class DahoasSyntheticinstructgptjpairwiseDataset(PromptRawDataset):
    ...
    def get_train_data(self):
        ...
        # save to json file
        # save_subset_to_json(dataset, self.output_path + self.dataset_name_clean + '/train.json')
        return dataset
    ...
    def get_eval_data(self):
        ...
        dataset = Subset(dataset, index)
        # save to json file
        # save_subset_to_json(dataset, self.output_path + self.dataset_name_clean + '/eval.json')
        return dataset

    def get_prompt(self, sample):
        #return " Human: " + sample['prompt'] + " Assistant:"
        return convert_to_llama_format(" Human: " + sample['prompt'])
    ...
    def get_prompt_and_chosen(self, sample):
        ...
        return convert_to_llama_format(" Human: " + sample['prompt'] + " Assistant: " + sample['chosen'])
...
r"""
Maybe we can modify this one so that we do not need a separate function for train_year1.json?
It is working now so I do not really want to change and optimize
"""
