        ...
        if self.train_phase == 1:
            return {
                ...
                "labels": torch.where(self.chosen_dataset[idx]["attention_mask"].bool(),
                                      self.chosen_dataset[idx]["labels"], -100)
            }
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
                         end_of_conversation_token, max_seq_len, sft_prompt_eval = False):  # adding sft_prompt_eval parameters, specific to our tasks
    prompt_dataset = []
    chosen_dataset = []
    reject_dataset = []
    if train_phase == 1:
        for i, tmp_data in enumerate(current_dataset):
            ...
            # TODO: test this one 
            if sft_prompt_eval:
                chosen_sentence = raw_dataset.get_prompt(tmp_data)
            else:
                chosen_sentence = raw_dataset.get_prompt_and_chosen(
                    tmp_data)  # the accept response
                prompt = raw_dataset.get_prompt(tmp_data)
            if chosen_sentence is not None:
                if not sft_prompt_eval:
                ...
                labels = chosen_sentence["input_ids"].clone()
                if not sft_prompt_eval:
...
def create_dataset(local_rank, dataset_name, data_split, output_path,
                   ...
                   max_seq_len, rebuild, sft_prompt_eval = False):
    ...
    train_dataset = create_dataset_split(train_dataset, raw_dataset,
                                         ...
                                         max_seq_len, sft_prompt_eval = False)      # set False for training
    ...
    if local_rank <= 0 and (buf_create_cache.item() != 0 or reload):
        ...
        if len(data_path) == 1:  # Single dataset.
            train_dataset, eval_dataset = create_dataset(
                ...
                max_seq_len,
                rebuild=reload, 
                sft_prompt_eval=sft_prompt_eval)
