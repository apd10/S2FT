...
def main():
    ...
    A = torch.rand(input_dim, lora_dim, dtype=torch.bfloat16).cuda()
    B = torch.rand(lora_dim, input_dim, dtype=torch.bfloat16).cuda()
    C = torch.rand(s2_dim, input_dim, dtype=torch.bfloat16).cuda()
